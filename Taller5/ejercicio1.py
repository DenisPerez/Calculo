...
Q, R = np.linalg.qr(A)
Be = B(A, np.identity(5), Q, R)
K_fro = np.linalg.cond(A, p = 'fro')
Norm_froB = np.linalg.cond( (np.identity(5) - np.dot(A,Be)), p = 'fro')
A_inv = sla.inv(A)
Norm_fro = np.linalg.cond( (np.identity(5) - np.dot(A,A_inv)), p = 'fro')
...
K_fro = np.linalg.cond(A, p = 'fro')
Q, R = np.linalg.qr(A)
Be = B(A, np.identity(20), Q, R)
Norm_froB = np.linalg.cond( (np.identity(20) - np.dot(A, Be)), p = 'fro')
A_inv = sla.inv(A)
Norm_fro = np.linalg.cond( (np.identity(20) - np.dot(A, A_inv)), p = 'fro')
